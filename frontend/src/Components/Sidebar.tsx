import { Collapse, CollapseProps, Layout, Space, Dropdown, Button, MenuProps } from "antd";
...
export default function Sidebar() {
  const { learningLevel, setLearningLevel, moduleName, moduleCode, lessonOutline, setLabTopic } = useModuleContext();
  ...
  function selectLearningLevel(e: AntDropdownEvent) {
    const newLearningLevel = LEARNING_LEVELS?.find(level => level?.key === Number(e.key));
  ...
  return (
    <Sider className="bg-white sticky top-0 flex flex-col gap-4 h-full pt-20 px-8 shadow-2xl overflow-y-auto" theme="light" width={420}>
